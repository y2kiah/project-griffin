...
/* global gl extensions string, provided by client program via set_gl_extensions_string() */
std::string gl_extensions;
/* replace all the GL_EXTENSION checks with this function - JK */
const char *find_gl_extension(const char* check_extension) {
	if (gl_extensions.empty()) {
		gl_extensions.assign((char const*)glGetString(GL_EXTENSIONS)); // doesn't work on OpenGL version >= 3.2 with core profile
	}
	return strstr(gl_extensions.c_str(), check_extension);
}
/* set the extensions string to check against */
void SOIL_set_gl_extensions_string(const char* str) { gl_extensions.assign(str); }
...
int query_NPOT_capability( void )
{
	...
	if( has_NPOT_capability == SOIL_CAPABILITY_UNKNOWN )
	{
		...
		if (NULL == find_gl_extension("GL_ARB_texture_non_power_of_two")) {
...
int query_tex_rectangle_capability( void )
{
	...
	{
		/*	we haven't yet checked for the capability, do so	*/
		if (	(NULL == find_gl_extension("GL_ARB_texture_rectangle"))
			&&	(NULL == find_gl_extension("GL_EXT_texture_rectangle"))
			&&	(NULL == find_gl_extension("GL_NV_texture_rectangle")))
		{
			/*	not there, flag the failure	*/
...
int query_cubemap_capability( void )
{
	...
	{
		/*	we haven't yet checked for the capability, do so	*/
		if (	(NULL == find_gl_extension("GL_ARB_texture_cube_map"))
			&&	(NULL == find_gl_extension("GL_EXT_texture_cube_map")))
		{
			/*	not there, flag the failure	*/
...
int query_DXT_capability( void )
{
	...
	{
		/*	we haven't yet checked for the capability, do so	*/
		if (NULL == find_gl_extension("GL_EXT_texture_compression_s3tc")) {
			/*	not there, flag the failure	*/
